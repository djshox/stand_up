...
export default class HomePage extends React.Component {
    ...
    componentWillMount() {
        fetch("http://localhost/stand/time_plan.php")
            .then((response)=>response.json())
                .then((responseJson)=>
                {
                   ...
                   //let time_id=responseJson[0].id;
                    this.setState({start_time: stt_time,
                    stop_time:stp_time});
                    this.testFunc();
        ...
        fetch("http://localhost/stand/get_videos.php")
            .then((response)=>response.json())
            .then((responseJson)=>
            {

               // let set_video_gimn=responseJson[0].name;
                 let set_gimn_url='http://localhost/stand/'+responseJson[0].url;

                // let set_video=responseJson[1].name;
                 let set_video_url='http://localhost/stand/'+responseJson[1].url;

console.log(set_gimn_url);
                this.setState({set_gimn_url: set_gimn_url,
...
/////////////////////////////////////////////////////////
    ...
    sendToPHP=(e)=>{
        ...
        axios.post("http://localhost/stand/temp.php",formData)
